...
/** Narrow windows = fewer HTTP calls per league; full window still covers near-term fixtures */
const SPORT_DATE_WINDOWS: Record<SportType, { pastDays: number; futureDays: number }> = {
  soccer: { pastDays: 1, futureDays: 2 },
  ncaa_football: { pastDays: 1, futureDays: 1 },
...
type FetchEspnOptions = { quick?: boolean };
...
async function fetchEspnGames(
  ...
  try {
    ...
    return uniqueEvents.map((event) => mapEspnEventToGame(event, league, sport));
  } catch (error: any) {
    console.error(
...
export async function fetchSoccerGames(
  ...
  if (!league) return [];
  return fetchEspnGames(leagueId, league, "soccer", options);
}

export async function fetchNcaaGames(
  leagueId: string,
  options?: FetchEspnOptions
): Promise<Game[]> {
  const league = NCAA_LEAGUES.find((l) => l.id === leagueId);
  if (!league) return [];
  return fetchEspnGames(leagueId, league, league.sport, options);
}

export type FetchAllGamesOptions = { quick?: boolean };
...
export async function fetchAllGames(
  ...
  const allPromises = [
    ...soccerLeagueIds.map((id) => fetchSoccerGames(id, { quick })),
    ...ncaaLeagueIds.map((id) => fetchNcaaGames(id, { quick })),
  ];
